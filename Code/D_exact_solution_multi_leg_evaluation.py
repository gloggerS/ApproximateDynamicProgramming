...
for capacities in var_capacities:
    for preferences_no_purchase in var_no_purchase_preferences:
        ...
        value_final['' + str(capacities) + '-' + str(preferences_no_purchase)] = pd.DataFrame(v_results[:, 0])
        capacities_final['' + str(capacities) + '-' + str(preferences_no_purchase)] = pd.DataFrame(c_results[:, -1, :])
        products_all['' + str(capacities) + '-' + str(preferences_no_purchase)] = p_results
        offersets_all['' + str(capacities) + '-' + str(preferences_no_purchase)] = o_results
