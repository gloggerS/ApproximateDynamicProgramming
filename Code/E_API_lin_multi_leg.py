...
for capacities in var_capacities:
    ...
    for preferences_no_purchase in var_no_purchase_preferences:
        ...
        for k in np.arange(K)+1:
            ...
            v_samples = np.zeros((I, len(times)+1))
            c_samples = np.zeros((I, len(times)+1, len(capacities)))

            for i in np.arange(I):
                # line 3
                r_sample_i = np.zeros(len(times)+1)  # will become v_sample
                c_sample_i = np.zeros(shape=(len(times)+1, len(capacities)), dtype=int)
